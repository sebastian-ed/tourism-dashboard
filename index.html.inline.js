...
function renderDashboard() {
  ...
  const selectedAnnualStats = calcStats(getAnnualValuesForYears(currentYearlyStats, shownYears));
  const selectedLabel = getSelectedYearsLabel(shownYears, years);
  const singleSelectedYear = shownYears.length === 1 ? shownYears[0] : null;
  const selectedAnnualValue = singleSelectedYear
    ? currentYearlyStats[singleSelectedYear]?.annualValue
    : selectedAnnualStats?.mean;

  const selectedKpis = [
    { label: singleSelectedYear ? `${annualMeta.shortLabel} ${singleSelectedYear}` : `Promedio ${annualMeta.shortLabel.toLowerCase()}`, value: formatNumber(selectedAnnualValue), sub: selectedLabel, accent: '#10B981' },
  ...
  const globalKpis = [
    ...
    { label: 'Desvío estándar global', value: formatNumber(globalStats?.stdDev), sub: 'Variabilidad total', accent: '#EAB308' },
    { label: 'Años cargados', value: years.length ? String(years.length) : '-', sub: years.length ? `${years[0]}–${years[years.length - 1]}` : 'Sin serie', accent: '#10B981' },
  ];
